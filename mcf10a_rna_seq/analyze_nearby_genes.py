...
u2os_red = u2os.loc[idx]

ratio = u2os_red['mean_tpm'] / mcf10A_red['mean_tpm']
u2os_red['ratio'] = ratio

ratio = ratio.replace(np.inf, 10 ** 4)
ratio.loc[(ratio == 0) | (ratio.isna())] = 10 ** -4
plotting.histplot(np.log10(ratio), 'log10(U2OS / MCF10A)', bin_count=200, xlims=[-4.1, 4.1])

# For genes are only expressed in u2os, what are their properties?
u2os_red.loc[u2os_red['ratio'] == np.inf, 'name']
# Problem here is that each gene can have multiple matches to sat3. take closest? Largest?
dist = np.empty([u2os_red.shape[0], 1])

for c, p in enumerate(u2os_red.iterrows()):
    this_name = p[1]['name']
    matches = genes_sel.loc[genes['names'] == this_name]
    # Closest match?
    dist[c]=matches['distance_to_repeat'].min()

u2os_red['distance_to_repeat_min']=dist
dist_exclusive_expression=u2os_red.loc[u2os_red['ratio'] == np.inf, 'distance_to_repeat_min'].values
